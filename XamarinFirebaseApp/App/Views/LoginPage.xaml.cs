    ...
    public partial class LoginPage : ContentPage
    {
        ...
        private async void BtnSignIn_Clicked(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    await DisplayAlert("Error", "Ingrese correo", "Ok");
                    return;
                }
                if (String.IsNullOrEmpty(password))
                {
                    await DisplayAlert("Error", "Ingrese password", "Ok");
                    return;
                }
            ...
            catch(Exception exception)
            {
                ...
                {
                    await DisplayAlert("Error", "Email no valido", "ok");
                }
                else if(exception.Message.Contains("INVALID_PASSWORD"))
                {
                    await DisplayAlert("Error", "Password incorrecto", "ok");
                }
                else
                {
                    await DisplayAlert("Error", "Email no valido", "ok");
